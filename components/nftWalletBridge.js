...
export default function NFTWalletBridge(e) {
    ...
    async function sendMint(Amount) {
        ...
        if (process.env.mintType == "Pre-Sale") {
            ...
            .whitelistMissingToothMint(false, process.env.messagehash, process.env.messagehash2, process.env.messagehash, process.env.messagehash2, process.env.signatureBytes2, Amount)
                .send({ from: connectedWalletAddress, value: bntokens })
                .on('transactionHash', function (hash) {
                    //hashArray = [];
